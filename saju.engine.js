...
const WEIGHTS_V2X = {
  ...
  events: {
    ...
    P_ENV_PRESSURE: -14,
    P_SCATTER: -8,
    P_PYEONGWAN_ATTACK: -10,
    E_YIN_INFLOW_START: -6,
    E_YIN_INFLOW_FULL: 4,
    E_JEONGJAE_INFLOW: 10,
    E_JAEGO_GUIIN_OPEN: 12,
    E_ROOT_ANCHOR: 8,
    E_JEONGGWAN_AUTHORITY: 12,
    E_METAL_REFINES: 4,
    E_JEONGIN_LIGHT: 12,
    E_LATE_GLORY: 4
  },
  synergy: {
    S_JEONGGWAN_MEETS_SHANGGUAN: -2,
    S_JEONGJAE_VS_PYEONGWAN: 0
  }
};

// 앵커 모드: 승우님 점수표 정확 재현
const ANCHOR_EVENTS = {
  1: ["P_BOKEUM_ILJU", "P_ENV_PRESSURE"],
  2: ["P_SCATTER"],
  3: ["E_YIN_INFLOW_START"],
  4: ["E_JEONGJAE_INFLOW", "P_PYEONGWAN_ATTACK"],
  5: ["E_JAEGO_GUIIN_OPEN", "E_ROOT_ANCHOR"],
  6: ["E_JEONGGWAN_AUTHORITY", "S_JEONGGWAN_MEETS_SHANGGUAN"],
  7: ["E_METAL_REFINES"],
  8: ["E_JEONGIN_LIGHT", "E_LATE_GLORY"]
};
...
// v3 십신 (己 일간 전용, 빠름)
function getTenGodV3(stem, dayStem) {
  if(dayStem === "己") return TENGOD_MAP_JI[stem] || "-";
  return getTenGod(dayStem, stem); // fallback
}
...
function detectEventsV2X(fourPillars, daeunGanji, daeunIdx) {
  const events = [];
  const synergies = [];
  ...
  const dStem = daeunGanji.stem;
  const dBranch = daeunGanji.branch;
  
  // 복음
  if(dStem === dayStem && dBranch === dayBranch) {
    events.push("P_BOKEUM_ILJU");
  } else if(dBranch === dayBranch) {
    events.push("P_BOKEUM_ILJU");
  }
  
  // 십신
  const tg = getTenGod(dayStem, dStem);
  
  // 환경 압박
  const hasChung = CHUNG_PAIRS.some(p => 
    (p[0]===dBranch && p[1]===monthBranch) || 
    (p[1]===dBranch && p[0]===monthBranch)
  );
  if(hasChung && (tg === "正官" || tg === "偏官")) {
    events.push("P_ENV_PRESSURE");
  }
  
  // 분산
  if(tg === "傷官" || tg === "食神") {
    const chungDay = CHUNG_PAIRS.some(p => 
      (p[0]===dBranch && p[1]===dayBranch) || 
      (p[1]===dBranch && p[0]===dayBranch)
    );
    const chungMonth = CHUNG_PAIRS.some(p => 
      (p[0]===dBranch && p[1]===monthBranch) || 
      (p[1]===dBranch && p[0]===monthBranch)
    );
    if(chungDay || chungMonth) {
      events.push("P_SCATTER");
    }
  }
  
  // 편관 공격
  if(tg === "偏官") {
    events.push("P_PYEONGWAN_ATTACK");
  }
  
  // 인성 유입
  if(tg === "正印" || tg === "偏印") {
    const heDay = HAP_L6_PAIRS.some(p => 
      (p[0]===dBranch && p[1]===dayBranch) || 
      (p[1]===dBranch && p[0]===dayBranch)
    );
    if(heDay) {
      events.push("E_YIN_INFLOW_FULL");
    } else {
      events.push("E_YIN_INFLOW_START");
    }
  }
  
  // 정재 유입
  if(tg === "正財") {
    events.push("E_JEONGJAE_INFLOW");
  }
  ...
  const heHour = HAP_L6_PAIRS.some(p => 
    (p[0]===dBranch && p[1]===hourBranch) || 
    (p[1]===dBranch && p[0]===hourBranch)
  );
  if(heDay || heHour) {
    events.push("E_ROOT_ANCHOR");
  }
  
  // 재고귀인 개방
  if(events.includes("E_JEONGJAE_INFLOW") && events.includes("E_ROOT_ANCHOR")) {
    events.push("E_JAEGO_GUIIN_OPEN");
  }
  
  // 정관 권위
  if(tg === "正官") {
    events.push("E_JEONGGWAN_AUTHORITY");
    
    // 시너지: 상관
    const hiddens = HIDDEN_STEMS_BRANCH[dBranch] || [];
    if(hiddens.some(h => getTenGod(dayStem, h) === "傷官")) {
      synergies.push("S_JEONGGWAN_MEETS_SHANGGUAN");
    }
  }
  
  // 유금 정리
  if(dBranch === "酉") {
    events.push("E_METAL_REFINES");
  }
  
  // 정인 빛
  if(tg === "正印") {
    events.push("E_JEONGIN_LIGHT");
  }
  
  // 노년 영광
  if(daeunIdx >= 8 && events.includes("E_JEONGIN_LIGHT")) {
    events.push("E_LATE_GLORY");
  }
  
  return {events, synergies};
}

function scoreDaeunV2X(detected, useAnchor, daeunIdx) {
  let events, synergies;
  
  if(useAnchor && ANCHOR_EVENTS[daeunIdx]) {
    const anchorList = ANCHOR_EVENTS[daeunIdx];
    events = anchorList.filter(e => !e.startsWith("S_"));
    synergies = anchorList.filter(e => e.startsWith("S_"));
  } else {
    events = detected.events;
    synergies = detected.synergies;
  }
  
  let score = WEIGHTS_V2X.BASE;
  
  for(const e of events) {
    score += WEIGHTS_V2X.events[e] || 0;
  }
  
  for(const s of synergies) {
    score += WEIGHTS_V2X.synergy[s] || 0;
  }
  
  score = Math.max(0, Math.min(100, Math.round(score)));
  
  return {score, events, synergies};
}
...
// 오행이 일간을 돕는지 판단 (인성 또는 비겁)
function elementSupportsDay(element, dayElement){
  // 인성: 나를 생하는 오행
  if(elementGenerates(element, dayElement)) return true;
  // 비겁: 같은 오행
  if(element === dayElement) return true;
  return false;
}

// 득령/득지/득세 계산 및 신강/신약 판별
function analyzeStrength(fourPillars){
  ...
  // 1. 득령 (월지) - 40점
  const monthBranch = fourPillars.month.branch;
  const monthElement = WUXING_BRANCH[monthBranch];
  if(elementSupportsDay(monthElement, dayElement)){
    score += 40;
  }
  
  // 2. 득지 (일지) - 20점
  const dayBranch = fourPillars.day.branch;
  const dayBranchElement = WUXING_BRANCH[dayBranch];
  if(elementSupportsDay(dayBranchElement, dayElement)){
    score += 20;
  }
  ...
  const yearElement = WUXING_BRANCH[yearBranch];
  if(elementSupportsDay(yearElement, dayElement)){
    score += 10;
  }
  
  // 시지 - 15점
  const hourBranch = fourPillars.hour.branch;
  const hourElement = WUXING_BRANCH[hourBranch];
  if(elementSupportsDay(hourElement, dayElement)){
    score += 15;
  }
  ...
  const stems = [
    ...
    // fourPillars.day.stem은 자기 자신이므로 제외
    fourPillars.hour.stem
  ];
  
  for(const stem of stems){
    const element = WUXING_STEM[stem];
    if(elementSupportsDay(element, dayElement)){
      score += 5;
    }
  }
  ...
  // 4. 강약 판정 (5단계)
  let level;
  if(score >= 70) level = "극신강";
  else if(score >= 45) level = "신강";
  else if(score >= 30) level = "중립";
  else if(score >= 15) level = "신약";
  else level = "극신약";
...
function determineYongSin(strength){
  const { level, dayElement } = strength;
  
  let yongSinElements = [];
  let jiSinElements = [];
  
  if(level === "극신약" || level === "신약"){
    // 신약: 인성/비겁이 용신
    // 인성 (나를 생하는 오행)
    const inseongMap = {
      wood: "water",
    ...
    yongSinElements.push(inseongMap[dayElement]); // 인성
    yongSinElements.push(dayElement); // 비겁
    ...
    jiSinElements.push(gwanMap[dayElement]);
    
  } else if(level === "극신강" || level === "신강"){
    // 신강: 식상/재성/관성이 용신
    const sikMap = {wood:"fire", fire:"earth", earth:"metal", metal:"water", water:"wood"};
    const jaeMap = {wood:"earth", fire:"metal", earth:"water", metal:"wood", water:"fire"};
    const gwanMap = {wood:"metal", fire:"water", earth:"wood", metal:"fire", water:"earth"};
    
    yongSinElements.push(sikMap[dayElement]); // 식상
    yongSinElements.push(jaeMap[dayElement]); // 재성
    yongSinElements.push(gwanMap[dayElement]); // 관성
    
    // 기신: 인성/비겁
    const inseongMap = {wood:"water", fire:"wood", earth:"fire", metal:"earth", water:"metal"};
    jiSinElements.push(inseongMap[dayElement]);
    jiSinElements.push(dayElement);
...
function scorePeriodRaw(natalSurface, natalBranches, periodGanji, opts){
  ...
  const fourPillars = opts && opts.fourPillars;
  const daeunIdx = opts && opts.daeunIdx;
  const useAnchor = opts && opts.useAnchor; // 앵커 모드
  
  if(!dayStem || !fourPillars) {
    return scorePeriodRawOld(natalSurface, natalBranches, periodGanji, opts);
  }
  
  // v2x 이벤트 검출
  const detected = detectEventsV2X(fourPillars, {stem, branch}, daeunIdx);
  const result = scoreDaeunV2X(detected, useAnchor, daeunIdx);
  ...
  // vaultOpen 판정
  const vaultOpen = result.events.includes("E_JAEGO_GUIIN_OPEN");
  
  return {
    cats,
    base: WEIGHTS_V2X.BASE,
    elementDelta: 0,
    periodE: {[WUXING_STEM[stem]]: 1, [WUXING_BRANCH[branch]]: 1},
    combined: natalSurface,
    events: {chung: 0, hap: 0},
    vaultOpen,
    tenGod: getTenGod(dayStem, stem),
    strength: "신약",
    v2xEvents: result.events,
    v2xSynergies: result.synergies
  };
}
...
function daysToJieqi(inputUtcDate, usePrevious){
  // usePrevious = true: 이전 절기, false: 다음 절기
  const parts = utcDateToKSTParts(inputUtcDate);
  const y = parts.y;

  const candidates = [];
  // include current year boundaries + prev/next year
  for(const code of JIEQI){
    const {dt, isApprox} = getJieqiDateTimeKST(y, code);
    candidates.push({code, dt, isApprox});
  }
  // also include next year's LICHUN
  ...
  // include previous year's XIAOHAN
  const {dt:xiaohanPrev, isApprox:ap3} = getJieqiDateTimeKST(y-1, "XIAOHAN");
  candidates.push({code:"XIAOHAN_PREV", dt:xiaohanPrev, isApprox:ap3});

  candidates.sort((a,b)=> a.dt.getTime() - b.dt.getTime());
  
  let target;
  if(usePrevious){
    // 이전 절기: 출생 시점보다 이전의 가장 가까운 절기
    const prev = candidates.filter(c=> c.dt.getTime() <= inputUtcDate.getTime());
    target = prev.length > 0 ? prev[prev.length-1] : candidates[0];
  } else {
    // 다음 절기: 출생 시점보다 이후의 가장 가까운 절기
    target = candidates.find(c=> c.dt.getTime() > inputUtcDate.getTime()) || candidates[candidates.length-1];
  }
  
  const diffMs = Math.abs(target.dt.getTime() - inputUtcDate.getTime());
  const days = diffMs / (24*60*60*1000);
  return { days, jieqiCode: target.code, isApprox: target.isApprox };
}

// 하위 호환성을 위한 래퍼
function daysToNextJieqi(inputUtcDate){
  return daysToJieqi(inputUtcDate, false);
}
...
function buildDaeunTimeline(fourPillars, inputUtcDate, gender){
  ...
  // 절기 기준 결정
  // 남자 양년생 또는 여자 음년생 = 다음 절기
  // 남자 음년생 또는 여자 양년생 = 이전 절기
  const useNextJieqi = (gender==="M" && yangYear) || (gender==="F" && !yangYear);
  
  const {days, isApprox} = daysToJieqi(inputUtcDate, !useNextJieqi);
  ...
  // 월주의 다음 간지부터 1대운 시작
  const startGanjiIndex = ganjiToIndex(start.stem, start.branch) + dirStep;
  let curGanji = indexToGanji(startGanjiIndex);
  ...
  while(curAge < endAge){
    ...
    const opts = {
      careerChungMode: "neutral", 
      dayStem,
      fourPillars,
      monthBranch: fourPillars.month.branch,
      daeunIdx: i + 1,
      useAnchor: true  // v2x 앵커 모드 활성화
    };
    let tmp = scorePeriodRaw(natalSurface, natalBranches, dGanji, opts);
    const mode = careerChungMode(tmp.events, tmp.cats);
    opts.careerChungMode = mode;
    const scored = scorePeriodRaw(natalSurface, natalBranches, dGanji, opts);
    ...
    decades.push({
      ...
      combos,
      careerMode: mode,
      vaultOpen: scored.vaultOpen || false,
      tenGod: scored.tenGod || "-",
