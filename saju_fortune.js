...
/* ---------------------------
   0-1) saju_core.js의 상수들을 사용 (중복 선언 제거)
   - WUXING_STEM, WUXING_BRANCH, WUXING_GENERATES, WUXING_CONTROLS
   - getShishen 함수
   모두 saju_core.js에 이미 선언되어 있으므로 여기서는 선언하지 않음
----------------------------*/
...
function getDaeunGrade(decade, fourPillars, natalBranches, natalSurface){
  ...
  console.log("🔥 초기 score:", score, "타입:", typeof score);
  ...
  console.log("🔥 용신:", yongshin, "천간오행:", decadeStemWuxing, "지지오행:", decadeBranchWuxing);
  
  if (decadeStemWuxing === yongshin || decadeBranchWuxing === yongshin) {
    score += 25;
    console.log(`✅ 용신 대운 +25: ${decade.stem}${decade.branch} (용신: ${yongshin})`);
  }
  
  console.log("🔥 용신 후 score:", score);
  ...
  console.log("🔥 보정 후 score:", score);
  
  // NaN 체크
  if (!Number.isFinite(score)) {
    console.error("⚠️⚠️⚠️ score is NaN or Infinity!!", score, decade);
  }
  ...
  console.log(`\n🎯🎯🎯 ${decade.stem}${decade.branch} 대운 최종 점수: ${score}점 (${grade}등급)\n`);
  
  return { grade, score };
}

// === 용신 판별 함수 (승우님 신약 무토 기준) ===
function determineYongshin(surface, fourPillars) {
  const dayStemWuxing = WUXING_STEM[fourPillars.day.stem];
  
  // 신약 무토(戊土) 사주는:
  // 용신(用神) = 비겁(土) - 나 자신을 강화
  // 희신(喜神) = 인성(火) - 나를 생해주는 최고 에너지
  
  // 일간이 토(土)면 토를 반환 (비겁이 용신)
  if (dayStemWuxing === "earth") {
    return "earth"; // 비겁(土)이 용신
  }
  
  // 일간이 약하면 일간을 생하는 오행이 희신
  const total = Object.values(surface).reduce((a, b) => a + b, 0);
  const dayStemCount = surface[dayStemWuxing];
  
  if (dayStemCount <= 2) {
    // 신약: 일간을 생하는 오행(인성)이 희신
...
// === 십신 점수 함수 (승우님 무토 일간 기준: 총 500점 배분) ===
function getShishenScore(shishen) {
  // 신약 무토(戊土) 기준 가중치
  // 인성(火) 150점: 최고 희신 - 추운 땅을 녹이는 필수 에너지
  // 비겁(土) 120점: 희신 - 내 뿌리, 자존감
  // 식상(金) 80점: 한신/기신 - 재주 부리지만 지치게 함
  // 관성(木) 80점: 기신 - 나무가 산을 파헤침
  // 재성(水) 70점: 기신 - 겨울 사주에 물이 얼어붙음
  
  const scores = {
    // 인성(印星) - 火 - 150점 (최고 희신)
    "正印": { score: +30, reason: "정인(화): 최고 희신, 학문/문서운" },
    "偏印": { score: +28, reason: "편인(화): 강한 희신, 직관력 증가" },
    
    // 비겁(比劫) - 土 - 120점 (희신)
    "比肩": { score: +24, reason: "비견(토): 희신, 자존감/동료운" },
    "劫財": { score: +20, reason: "겁재(토): 희신이나 경쟁 주의" },
    
    // 식상(食傷) - 金 - 80점 (한신/기신)
    "食神": { score: +8, reason: "식신(금): 재능 발휘하나 설기" },
    "傷官": { score: +5, reason: "상관(금): 표현력이나 지침" },
    
    // 관성(官星) - 木 - 80점 (기신)
    "正官": { score: -5, reason: "정관(목): 명예 vs 압박감" },
    "偏官": { score: -8, reason: "편관(목): 스트레스/압박 증가" },
    
    // 재성(財星) - 水 - 70점 (기신)
    "正財": { score: -10, reason: "정재(수): 재물이나 땅이 얼어붙음" },
    "偏財": { score: -12, reason: "편재(수): 재물욕심에 소진" }
  };
...
function renderFortuneAnalysis(){
  console.log("🔥🔥🔥 renderFortuneAnalysis CALLED 🔥🔥🔥", new Date().toISOString());
  console.log("🔥 window.SajuResult 존재?", !!window.SajuResult);
  
  // window.SajuResult가 없으면 대기
  if(!window.SajuResult){
    console.log("⚠️ SajuResult not ready yet...");
    return;
  }
  
  const result = window.SajuResult;
  console.log("🔥 SajuResult.daeunTimeline?", !!result.daeunTimeline);
  console.log("🔥 decades 배열?", result.daeunTimeline?.decades);
  console.log("🔥 decades 길이?", result.daeunTimeline?.decades?.length);
  
  // 대운 리스트에 총운 정보 추가
  const daeunListEl = document.getElementById("daeunList");
  console.log("🔥 daeunList element?", daeunListEl);
  
  if(!daeunListEl) {
    console.log("❌❌❌ daeunList 요소를 찾을 수 없음!!");
    return;
  }
  
  // 기존 대운 카드들을 업데이트
  const cards = daeunListEl.querySelectorAll(".daeun-card");
  console.log(`🔥 대운 카드 개수: ${cards.length}`);
  console.log(`🔥 대운 데이터 개수: ${result.daeunTimeline.decades.length}`);
  console.log("🔥 실제 카드들:", cards);
  
  result.daeunTimeline.decades.forEach((decade, idx) => {
    console.log(`\n🔥🔥 [대운 ${idx}] 처리 시작:`, decade);
    
    if(idx >= cards.length) {
      console.log(`⚠️ 카드 없음 (idx=${idx}, cards.length=${cards.length})`);
      return;
    }
    
    const card = cards[idx];
    console.log(`🔥 카드 ${idx} 찾음:`, card);
    
    // 등급 계산 (Gemini 정교화 로직)
    console.log(`\n📊 ${decade.stem}${decade.branch} 대운 분석 시작...`);
    const result_calc = getDaeunGrade(decade, result.fourPillars, result.natalBranches, result.natalSurface);
    const grade = result_calc.grade;
    const score = result_calc.score;
    
    console.log(`🔥 계산 완료 - grade: ${grade}, score: ${score}`);
    
    // 점수 표시 추가 (기존 것이 있으면 제거 후 재생성)
    const header = card.querySelector(".daeun-header");
    console.log(`🔥 header 찾기:`, header);
    
    if(header){
      // 기존 점수 요소 제거
      const existingScore = header.querySelector(".daeun-score");
      console.log(`🔥 기존 점수 요소:`, existingScore);
      
      if(existingScore){
        existingScore.remove();
        console.log(`🔥 기존 점수 제거함`);
      }
      ...
      console.log(`✅✅✅ ${decade.stem}${decade.branch} 점수 표시 완료: ${score}점 (${grade}등급)`);
      console.log(`🔥 생성된 scoreEl:`, scoreEl);
      console.log(`🔥 header.innerHTML:`, header.innerHTML);
    } else {
      console.error(`❌❌❌ header 없음! 카드 ${idx}`);
    }
    
    // 총운 요약 추가 (기존 것이 있으면 제거 후 재생성)
...
// renderFortuneAnalysis 함수를 전역으로 노출 (saju_core.js에서 호출 가능하도록)
