...
function renderFortuneAnalysis(){
  ...
  console.log("✅ SajuResult 있음:", window.SajuResult);
...
/* ---------------------------
   3) 초기화 및 감시
----------------------------*/
// SajuResult가 준비되면 자동으로 실행 (백업용 - core에서 직접 호출하므로 필요성 감소)
let fortuneCheckCount = 0;
let checkInterval = setInterval(() => {
  fortuneCheckCount++;
  if(window.SajuResult){
    console.log("✅ SajuResult 감지! renderFortuneAnalysis 실행");
    renderFortuneAnalysis();
    clearInterval(checkInterval);
  } else if (fortuneCheckCount > 50) {
    console.warn("⚠️ SajuResult를 찾을 수 없습니다.");
    clearInterval(checkInterval);
  }
}, 100);
...
document.addEventListener("DOMContentLoaded", ()=>{
  setTimeout(() => {
    console.log("DOM 로드 완료, SajuResult 체크:", !!window.SajuResult);
    if(window.SajuResult){
      renderFortuneAnalysis();
    }
  }, 500);
