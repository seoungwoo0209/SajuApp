...
function buildDaeunTimeline(fourPillars, inputUtcDate, gender){
  ...
  while(curAge < endAge){
    ...
    curAge = nextAge;
    curGanji = indexToGanji(curGanji.index + 10*dirStep);
  }
...
function computeLifeScoreFromYears(years, birthUtc){
  const birthY = utcDateToKSTParts(birthUtc).y;
  ...
  for(const y of years){
    const age = y.year - birthY;
    const w = weightOfAge(age);
    sum += y.total * w;
    wsum += w;
  }
  return Math.round(sum / Math.max(1e-9, wsum));
...
function runAll(){
  ...
  try{
    ...
    const lifeScore = computeLifeScoreFromYears(years, birthUtc);
