...
function runAll(){
  ...
  try{
    ...
    // 대운/세운 아코디언
    const lifeRoot = $("daeunTimeline");
    if(lifeRoot){
      const seedBase = `${birthDate}|${birthTime}|${gender}|${name}`;

      // meta + badges
      const metaEl = $("daeunMeta");
      if(metaEl){
        const approxTxt = dt.daeunStart?.isApprox ? " (절기근사모드)" : "";
        metaEl.textContent = `대운 시작: ${dt.daeunStart.age}세 / ${dt.daeunStart.dateApprox} / 방향: ${dt.direction}${approxTxt}`;
      }
      const badgesEl = $("lifeBadges");
      if(badgesEl){
        const volPack = vol ? buildGlobalBadges(lifeScore, vol) : buildGlobalBadges(lifeScore, {label:"Normal"});
        renderBadges(badgesEl, volPack);
      }

      // A안: 10년 대운만 고정 리스트로 출력(클릭/아코디언 없음)
      renderDecadeOnlyList(lifeRoot, dt.decades, seedBase);
    }
...
/* --------------------------- 14-1) 대운 10년 고정 리스트(A안) ---------------------------- */
function renderDecadeOnlyList(root, decades, seedBase){
  if(!root) return;
  if(!Array.isArray(decades) || decades.length===0){
    root.innerHTML = `<div class="tiny muted">대운 데이터가 없습니다.</div>`;
    return;
  }

  const lines = decades.map((d, i)=>{
    const band = (typeof commonBandPhrase==="function") ? commonBandPhrase(seedBase+`|decade|${i}`, d.grade) : "";
    const combo = (d.combos && d.combos.length) ? ` · ${d.combos.slice(0,2).join(", ")}` : "";
    const badges = buildBadgesForPeriod(d);
    const badgeHtml = (badges||[]).map(b=>`<span class="badge ${b.cls||""}">${escapeHtml(b.t)}</span>`).join("");

    return `
      <div class="row" style="margin-top:10px;">
        <div class="miniCard" style="width:100%;">
          <div class="accTitle">【${escapeHtml(d.label)} / ${escapeHtml(d.ganji)}】 ${d.total}점(${d.grade})</div>
          <div class="badgeRow" style="margin-top:6px;">${badgeHtml}</div>
          <div class="kvs" style="margin-top:8px;">
            <span class="kv">연애 ${d.cats.love}</span>
            <span class="kv">재물 ${d.cats.money}</span>
            <span class="kv">커리어 ${d.cats.career}</span>
            <span class="kv">건강 ${d.cats.health}</span>
          </div>
          <div class="tiny muted" style="margin-top:8px;">${escapeHtml(band)}${escapeHtml(combo)}</div>
        </div>
      </div>
    `;
  }).join("");
