...
function renderDaeunDecadeList(root, decades, seedBase){
  // 10년 단위 대운만 간단히 표시 (요청사항)
  root.innerHTML = "";
  const wrap = document.createElement("div");
  wrap.className = "daeunList";

  decades.forEach((d, i)=>{
    const item = document.createElement("div");
    item.className = "daeunRow";
    ...
    item.innerHTML = `
      <div class="daeunHead">${escapeHtml(title)}</div>
      <div class="tiny muted" style="margin-top:6px;">${escapeHtml(one)}</div>
      <div class="tiny" style="margin-top:8px;">${escapeHtml(detail)}</div>
    `;
