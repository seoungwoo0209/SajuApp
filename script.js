...
function buildDaeunTimeline(fourPillars, inputUtcDate, gender){
  ...
  while(curAge < endAge){
    ...
    let tmp = scorePeriodRaw(natalSurface, natalBranches, dGanji, {careerChungMode:"neutral", dayStem});
    const mode = careerChungMode(tmp.events, tmp.cats);
    const scored = scorePeriodRaw(natalSurface, natalBranches, dGanji, {careerChungMode: mode, dayStem});
...
function renderDaeunList(root, decades, birthUtc, dayStem){
  ...
  decades.forEach((dec, idx)=>{
    ...
    // 대운 천간의 십신
    const tenGod = getTenGod(dayStem, dec.stem);
