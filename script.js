...
function scorePeriodRaw(natalSurface, natalBranches, periodGanji, opts){
  ...
  const target = 2;
  const dayElement = dayStem ? WUXING_STEM[dayStem] : null;
  ...
  for(const k of Object.keys(deficit)){
    ...
    if(k === dayElement){
      excess[k] = Math.max(0, (natalSurface[k]||0) - 3.5);
    } else {
  ...
  // 십신 보정 강화
  let tenGodDelta = {money:0, career:0, love:0, health:0};
  let vaultOpen = false;
  let strength_level = "중립";
  ...
  if(dayStem && fourPillars){
    ...
    if(strength.level === "극신약" || strength.level === "신약"){
      // 신약: 인성/비겁 크게 가산
      if(tg === "正印" || tg === "偏印"){
        tenGodDelta.money = 15;
        tenGodDelta.career = 20;
        tenGodDelta.love = 10;
        tenGodDelta.health = 15;
      } else if(tg === "比肩" || tg === "劫財"){
        tenGodDelta.money = 5;
        tenGodDelta.career = 15;
        tenGodDelta.love = 5;
        tenGodDelta.health = 15;
      } else if(tg === "正官" || tg === "偏官"){
        tenGodDelta.money = -5;
        tenGodDelta.career = -10;
        tenGodDelta.love = 5;
        tenGodDelta.health = -15;
      } else if(tg === "食神"){
        tenGodDelta.money = 10;
        tenGodDelta.career = 10;
        tenGodDelta.love = 5;
        tenGodDelta.health = 0;
      } else if(tg === "傷官"){
        tenGodDelta.money = 8;
        tenGodDelta.career = 5;
        tenGodDelta.love = 0;
        tenGodDelta.health = -5;
      } else if(tg === "正財" || tg === "偏財"){
        tenGodDelta.money = 15;
        tenGodDelta.career = 5;
        tenGodDelta.love = 10;
    ...
    if(vault && ev.chung > 0){
      ...
      if(hasVault && chungPairs){
        vaultOpen = true;
        // 재물 창고 개방 시 모든 카테고리에 큰 가산
        tenGodDelta.money += 30;
        tenGodDelta.career += 25;
        tenGodDelta.love += 20;
        tenGodDelta.health += 15;
      }
    }
  ...
  for(const c of ["love","money","career","health"]){
    ...
    // Career chung opportunity mode
    let chungExtra = 0;
    if(ev.chung>0 && !vaultOpen){ // 재고 개방이 아닐 때만 패널티
