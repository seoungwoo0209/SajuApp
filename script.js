...
function scorePeriodRaw(natalSurface, natalBranches, periodGanji, opts){
  ...
  const monthBranch = opts && opts.monthBranch;
  ...
  if(strength.level === "극신약" || strength.level === "신약"){
    // 신약: 인성/비겁 용신
    if(tenGod === "正印" || tenGod === "偏印") tenGodBonus = 20;
    else if(tenGod === "比肩" || tenGod === "劫財") tenGodBonus = 15;
    else if(tenGod === "正官" || tenGod === "偏官") tenGodBonus = -20;
    else if(tenGod === "食神" || tenGod === "傷官") tenGodBonus = -5;
    else if(tenGod === "正財" || tenGod === "偏財") tenGodBonus = -5;
  } else if(strength.level === "극신강" || strength.level === "신강"){
    // 신강: 식상/재성/관성 용신
    if(tenGod === "正財" || tenGod === "偏財") tenGodBonus = 20;
    else if(tenGod === "食神" || tenGod === "傷官") tenGodBonus = 15;
    else if(tenGod === "正官" || tenGod === "偏官") tenGodBonus = 10;
    else if(tenGod === "正印" || tenGod === "偏印") tenGodBonus = -15;
    else if(tenGod === "比肩" || tenGod === "劫財") tenGodBonus = -15;
  } else {
    // 중립
    if(tenGod === "正財" || tenGod === "偏財") tenGodBonus = 10;
    else if(tenGod === "食神" || tenGod === "傷官") tenGodBonus = 5;
    else if(tenGod === "正印" || tenGod === "偏印") tenGodBonus = 5;
  }
  ...
  if(monthBranch){
    ...
    if(season === "winter" && periodStemElement === "fire"){
      baseScore += 10; // 겨울에 화는 귀함
    } else if(season === "summer" && periodStemElement === "water"){
      baseScore += 10; // 여름에 수는 귀함
    }
  }
  ...
  // 8. 합/삼합/방합 (긍정)
  baseScore += 5 * ev.hap;
  baseScore += 8 * ev.samhahp;
  baseScore += 8 * ev.banghap;
  
  // 9. 형/파/해 (부정)
  baseScore -= 4 * ev.hyeong;
  baseScore -= 3 * ev.pa;
  baseScore -= 3 * ev.hae;
  
  // 10. 최종 점수 계산
  let finalScore = baseScore * vaultMultiplier;
  ...
  // 카테고리별 점수 (간단 버전)
  const cats = {
    love: clamp(Math.round(finalScore + (tenGod.includes("財") || tenGod.includes("官") ? 5 : 0)), 0, 100),
    money: clamp(Math.round(finalScore + (tenGod.includes("財") ? 10 : 0)), 0, 100),
    career: clamp(Math.round(finalScore + (tenGod.includes("官") || tenGod.includes("印") ? 5 : 0)), 0, 100),
    health: clamp(Math.round(finalScore), 0, 100)
  };
  
  return { 
    cats, 
    base: baseScore, 
    elementDelta: tenGodBonus,
    periodE: {[periodStemElement]: 1, [periodBranchElement]: 1},
    combined: natalSurface,
    events: ev,
    vaultOpen: vaultMultiplier > 1.0,
    tenGod: tenGod,
...
function buildDaeunTimeline(fourPillars, inputUtcDate, gender){
  ...
  // 대운 방향 결정 (임시: 강제 순행 테스트)
  const direction = "순행"; // 테스트용
  // const direction = (() => {
