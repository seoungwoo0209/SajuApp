...
function scorePeriodRaw(natalSurface, natalBranches, periodGanji, opts){
  ...
  const dayElement = WUXING_STEM[dayStem];
  const periodStemElement = WUXING_STEM[stem];
  const periodBranchElement = WUXING_BRANCH[branch];
  
  // 1. 신강/신약 분석
  const strength = analyzeStrength(fourPillars);
  const isWeak = (strength.level === "극신약" || strength.level === "신약");
  const isStrong = (strength.level === "극신강" || strength.level === "신강");
  ...
  // 오행 균형도 체크
  let balanceScore = 0;
  for(const [k, v] of Object.entries(natalSurface)){
    const diff = Math.abs(v - 2);
    balanceScore -= diff * 2;
  }
  baseScore += Math.max(-20, balanceScore);
  
  // 4. 대운 오행이 용신인지 기신인지 판단
  let elementScore = 0;
  
  if(isWeak){
    // 신약: 인성(나를 생) / 비겁(같은 오행) = 용신
    if(elementGenerates(periodStemElement, dayElement)){
      elementScore += 12; // 천간 인성
    }
    if(periodStemElement === dayElement){
      elementScore += 8; // 천간 비겁
    }
    if(elementGenerates(periodBranchElement, dayElement)){
      elementScore += 15; // 지지 인성
    }
    if(periodBranchElement === dayElement){
      elementScore += 12; // 지지 비겁
    }
    
    // 기신: 관성(나를 극) / 재성(내가 극)
    if(elementControls(periodStemElement, dayElement)){
      elementScore -= 10; // 천간 관성
    }
    if(elementControls(dayElement, periodStemElement)){
      elementScore -= 5; // 천간 재성
    }
    if(elementControls(periodBranchElement, dayElement)){
      elementScore -= 15; // 지지 관성
    }
    if(elementControls(dayElement, periodBranchElement)){
      elementScore -= 8; // 지지 재성
    }
    
  } else if(isStrong){
    // 신강: 식상(내가 생) / 재성(내가 극) / 관성(나를 극) = 용신
    if(elementGenerates(dayElement, periodStemElement)){
      elementScore += 15; // 천간 식상
    }
    if(elementControls(dayElement, periodStemElement)){
      elementScore += 12; // 천간 재성
    }
    if(elementControls(periodStemElement, dayElement)){
      elementScore += 10; // 천간 관성
    }
    if(elementGenerates(dayElement, periodBranchElement)){
      elementScore += 18; // 지지 식상
    }
    if(elementControls(dayElement, periodBranchElement)){
      elementScore += 15; // 지지 재성
    }
    if(elementControls(periodBranchElement, dayElement)){
      elementScore += 12; // 지지 관성
    }
  ...
  let eventScore = 0;
  eventScore += ev.hap * 5;
  eventScore += ev.samhahp * 8;
  eventScore += ev.banghap * 8;
  eventScore -= ev.hyeong * 6;
  eventScore -= ev.pa * 4;
  eventScore -= ev.hae * 3;
  ...
  let vaultBonus = 0;
  let vaultOpen = false;
  const vault = VAULT_MAP[dayElement];
  
  if(vault && ev.chung > 0){
    const hasVault = natalBranches.includes(vault);
    const chungPairs = CHUNG_PAIRS.find(pair => 
      (pair[0] === vault && pair[1] === branch) || 
      (pair[1] === vault && pair[0] === branch)
    );
    
    if(hasVault && chungPairs){
      vaultOpen = true;
      vaultBonus = 20; // 재고 개방 보너스
      eventScore += ev.chung * 5; // 충 패널티 상쇄
    } else {
      eventScore -= ev.chung * 8; // 일반 충 패널티
    }
  } else if(ev.chung > 0){
    eventScore -= ev.chung * 8;
  }
  
  // 7. 최종 점수 계산
  let totalScore = baseScore + elementScore + eventScore + vaultBonus;
  ...
  const cats = {
    love: totalScore,
    money: Math.round(clamp(totalScore + (vaultOpen ? 5 : 0), 0, 100)),
    career: totalScore,
    health: totalScore
  };
  ...
  return { 
    cats, 
    base: baseScore, 
    elementDelta: elementScore,
    periodE: {[periodStemElement]: 1, [periodBranchElement]: 1},
