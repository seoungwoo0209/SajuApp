...
function renderLifeAccordion(root, ctx){
  // Option A: 10년 단위(대운) 아코디언만 렌더링 (index.html의 #daeunTimeline에 맞춤)
  if(!root) return;

  const decades = Array.isArray(ctx?.decades) ? ctx.decades : [];
  if(decades.length === 0){
    root.innerHTML = `<div class="muted">대운 데이터를 만들 수 없습니다.</div>`;
    return;
  }

  const html = decades.map((d, idx) => {
    const ageFrom = d.ageFrom ?? "-";
    const ageTo = d.ageTo ?? "-";
    const ganji = d.ganji ?? "";
    const score = (typeof d.score === "number") ? d.score : (d.scoreText ?? "-");
    const title = `${ageFrom}세~${ageTo}세`;
    const meta = `${ganji ? ganji + " · " : ""}${score}점`;

    const summary = d.summary ? escapeHtml(String(d.summary)) : "—";
    return `
      <div class="accItem" data-idx="${idx}">
        <div class="accHead" role="button" tabindex="0" aria-expanded="false">
          <div>
            <div class="accTitle">${escapeHtml(title)}</div>
            <div class="accSub">${escapeHtml(meta)}</div>
          </div>
          <div class="accChev">▾</div>
        </div>
        <div class="accBody" hidden>
          <div class="muted">${summary}</div>
          <div class="muted" style="margin-top:8px;">(클릭 시 상세 확장 — 향후 유료/잠금 가능)</div>
        </div>
      </div>
    `;
  }).join("");

  root.innerHTML = `<div class="accordion">${html}</div>`;

  function toggle(item){
    const head = item.querySelector(".accHead");
    const body = item.querySelector(".accBody");
    const chev = item.querySelector(".accChev");
  ...
  root.addEventListener("click", (e) => {
    ...
    const item = head.closest(".accItem");
    if(item) toggle(item);
  }, { passive: true });
...
function runAll(){
  ...
  try{
    ...
    // 대운/세운 아코디언
    const lifeRoot = $("daeunTimeline") || $("lifeAccordion");
    if(lifeRoot){
      const ctx = {
...
function bindUI(){
  ...
  if(btnEx) btnEx.addEventListener("click", ()=>{
    // 기본 예시: 추승우 (1985-02-09 01:40 남)
    const setVal=(id,v)=>{ const el=$(id); if(el) el.value=v; };
    setVal("name","추승우");
    setVal("birthDate","1985-02-09");
    setVal("birthTime","01:40");
    setVal("gender","M");
    runAll();
  });

  const btnTests = $("btnTests");
  if(btnTests) btnTests.addEventListener("click", ()=>{
    try{
      runTests();
      showTopError("테스트 완료: 콘솔(F12) 확인");
    }catch(e){
      console.error(e);
      showTopError("테스트 실패: 콘솔(F12) 확인");
    }
  });
