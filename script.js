...
function fullWuxingSummary(surface){
  const order = ["wood","fire","earth","metal","water"]; // 목화토금수
...
function renderHiddenList(el, fourPillars){
  ...
  const items = [
    ...
    ["월지", fourPillars.month.branch],
    ["일지", fourPillars.day.branch],
    ["시지", fourPillars.hour.branch]
  ];
  el.innerHTML = items.map(([k,b])=>{
    const hs = HIDDEN_STEMS_BRANCH[b].join(" ");
    return `<div class="hiddenrow"><div class="k">${k} ${b}</div><div class="v">${hs}</div></div>`;
  }).join("");
}
...
function animatePillars(pillars){
  ...
  seq.forEach(([id,_])=> $(id).textContent = "-");
  seq.forEach(([id,val],i)=>{
    setTimeout(()=> { $(id).textContent = val; }, 600*i);
  });
}
...
function runAll(){
  ...
  setText("dayMaster", `일간: ${fourPillars.day.stem}`);

  const surface = getWuxingCounts(fourPillars, false);
  setText("top2", fullWuxingSummary(surface));
  renderBars($("barsSurface"), surface);

  // 지장간 탭
  renderHiddenList($("hiddenList"), fourPillars);
  const hiddenPack = getWuxingCounts(fourPillars, true);
  renderBars($("barsHidden"), hiddenPack.withHidden);

  // 대운/인생총운
  const luck = buildDaeunTimeline(fourPillars, birthUtc, gender);
  const natalBranches = [fourPillars.year.branch, fourPillars.month.branch, fourPillars.day.branch, fourPillars.hour.branch];
  const natalSurface = surface;

  const built = buildYearsAndMonths(natalSurface, natalBranches, luck.decades, birthUtc, {});
  const years = built.years;

  // Map for decade/year indexing
  const birthParts = utcDateToKSTParts(birthUtc);
  const decadeIndex = new Map();
  for(const d of luck.decades){
    const y0 = birthParts.y + d.startAge;
    const y1 = birthParts.y + d.endAge;
    for(let y=y0; y<=y1; y++) decadeIndex.set(y, d);
  }
  ...
  for(const [k, arr] of yearsByDecade.entries()){
    ...
    setText("lifeScore", String(lifeScore));

    // volatility from year totals
    const yearTotals = years.map(x=>x.total);
    const vol = computeVolatility(yearTotals);
    setText("volScore", String(vol.volatilityScore));
    setText("volLabel", vol.label);
    ...
    // daeun start info
    setText("daeunStart", `${luck.daeunStart.age}세(약 ${luck.daeunStart.dateApprox})`);
    setText("daeunDir", luck.direction);
