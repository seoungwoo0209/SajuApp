...
function renderLifeDecadeListFlat(root, ctx){
  if(!root) return;
  // ctx.daeun.decades: [{startAge,endAge,ganji:{stem,branch}, total, ...}]
  const decades = (ctx && ctx.daeun && Array.isArray(ctx.daeun.decades)) ? ctx.daeun.decades : [];
  if(decades.length===0){
    root.innerHTML = '<div class="muted">대운 데이터가 없습니다.</div>';
    return;
  }

  // Simple, non-collapsible list (옵션 A: 10년 단위만)
  let html = '<div class="life-decade-list">';
  for(const d of decades){
    const ageTxt = `${d.startAge}세~${d.endAge}세`;
    const ganjiTxt = (d.ganji && d.ganji.stem && d.ganji.branch) ? `${d.ganji.stem}${d.ganji.branch}` : '-';
    const score = (typeof d.total === 'number') ? Math.round(d.total) : '-';
    // 1줄 코멘트(간단): 점수 기반으로만
    let comment = '';
    if(typeof d.total === 'number'){
      if(d.total >= 70) comment = '상승/확장 운';
      else if(d.total >= 55) comment = '무난/유지 운';
      else if(d.total >= 40) comment = '주의/조정 운';
      else comment = '리스크 관리 운';
    }
    html += `
      <div class="life-decade-row">
        <div class="life-decade-left">
          <div class="life-decade-age">${ageTxt}</div>
          <div class="life-decade-ganji">${ganjiTxt} 대운</div>
        </div>
        <div class="life-decade-right">
          <div class="life-decade-score">${score}점</div>
          <div class="life-decade-comment">${comment}</div>
        </div>
      </div>`;
  }
  html += '</div>';

  // Minimal inline styles (avoid touching style.css). Kept tiny and scoped.
  html += `
  <style>
    .life-decade-list{display:flex;flex-direction:column;gap:10px;}
    .life-decade-row{display:flex;justify-content:space-between;align-items:center;
      border:1px solid rgba(255,255,255,.08); border-radius:14px; padding:12px 14px;
      background: rgba(0,0,0,.08);
    }
    .life-decade-age{font-weight:700; letter-spacing:-.2px;}
    .life-decade-ganji{opacity:.85; font-size:13px; margin-top:2px;}
    .life-decade-right{text-align:right}
    .life-decade-score{font-weight:800}
    .life-decade-comment{opacity:.8; font-size:12px; margin-top:2px;}
  </style>`;
