...
function renderDaeunDecadeList(root, decades, seedBase){
  // 옵션 A: 10년 단위 대운만 "클릭 펼침" 아코디언으로 표시
  root.innerHTML = "";

  const wrap = document.createElement("div");
  wrap.className = "acc"; // style.css의 accordion 스타일 재사용(없어도 동작)

  decades.forEach((d, i)=>{
    const badges = buildBadgesForPeriod(d);
    const headBadges = (badges||[]).map(b=>`<span class="badge ${b.cls||""}">${escapeHtml(b.t)}</span>`).join("");

    const item = document.createElement("div");
    item.className = "accItem";

    const title = `【${d.startAge}~${d.endAge}세 / ${d.ganji}】 ${d.total}점(${d.grade})`;

    item.innerHTML = `
      <div class="accHead" role="button" tabindex="0">
        <div class="accTitle">${escapeHtml(title)}</div>
    ...
    const renderBody = ()=>{
      ...
      const one = commonBandPhrase(seedBase+"|decade|"+i, d.grade);
      const catLine = `연애 ${d.cats.love} · 재물 ${d.cats.money} · 직장 ${d.cats.career} · 건강 ${d.cats.health}`;

      // 콤보/모드 요약(있으면)
      const comboTxt = (d.combos && d.combos.length)
        ? d.combos.map(k=> (KO.comboHints && KO.comboHints[k]) ? `${k}${KO.comboHints[k]}` : k).join(", ")
        : "";

      const extra = [];
      if(d.careerMode && d.careerMode!=="none" && KO.careerChungModes && KO.careerChungModes[d.careerMode]){
        extra.push(pickDeterministic(KO.careerChungModes[d.careerMode], seedBase+"|careerMode|"+i));
      }
      if(d.overheat && KO.overheatPhrases && KO.overheatPhrases.text){
        extra.push(pickDeterministic(KO.overheatPhrases.text, seedBase+"|overheat|"+i));
      }

      body.innerHTML = `
        <div class="row">
          <div class="miniCard">
            <div class="miniTitle">요약</div>
            <div class="tiny">${escapeHtml(one)}</div>
            ${comboTxt ? `<div class="tiny muted" style="margin-top:6px;">${escapeHtml(comboTxt)}</div>` : ""}
          </div>
          <div class="miniCard">
            <div class="miniTitle">카테고리 점수</div>
            <div class="kvs">
              <span class="kv">연애 ${d.cats.love}</span>
              <span class="kv">재물 ${d.cats.money}</span>
              <span class="kv">직장 ${d.cats.career}</span>
              <span class="kv">건강 ${d.cats.health}</span>
            </div>
            <div class="tiny muted" style="margin-top:6px;">${escapeHtml(catLine)}</div>
          </div>
        </div>
        ${extra.length ? `<div class="row"><div class="tiny muted">${escapeHtml(extra.join(" "))}</div></div>` : ""}
      `;
    };

    const toggle = ()=>{
      item.classList.toggle("open");
      if(item.classList.contains("open")) renderBody();
    };
...
// find first element whose textContent includes given needle
function findFirstTextElement(needle){
  try{
    const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_ELEMENT, {
      acceptNode(node){
        const t = (node.textContent||"").trim();
        if(!t) return NodeFilter.FILTER_SKIP;
        // prefer leaf-ish nodes (avoid massive containers)
        if(t.includes(needle) && t.length <= needle.length + 30) return NodeFilter.FILTER_ACCEPT;
...
function ensureChipsContainer(label){
  const id = (label==="best") ? "bestYears" : "worstYears";
  let el = document.getElementById(id);
  if(el) return el;

  const labelText = (label==="best") ? "베스트 3년" : "주의 3년";
  const labEl = findFirstTextElement(labelText);
  if(!labEl) return null;

  const div = document.createElement("div");
  div.id = id;
  div.className = "chips";
  // try to insert right after label node
  labEl.insertAdjacentElement("afterend", div);
  return div;
}

function ensureLifeAccordionRoot(){
  let root = getElAny(["lifeAccordion","daeunAccordion","daeunTimeline","lifeTimeline"]);
  if(root) return root;

  const titleEl = findFirstTextElement("대운 타임라인");
  if(!titleEl) return null;

  // choose the big timeline card/container
  const container = titleEl.closest(".miniCard, .card, .panel, .section, .tabpane") || titleEl.parentElement;
  const div = document.createElement("div");
  div.id = "lifeAccordion";
...
function runAll(){
  ...
  try{
    ...
    // LifeScore는 "대운 평균" 성격으로만 유지 (세운/월운은 생략)
    const lifeScore = computeLifeScore(dt.decades);
    safeSetText("lifeScore", String(lifeScore));

    // 변동성/Top3(연도)는 세운을 끈 상태이므로 비움
    safeSetText("volScore", "-");
    const bestEl = $("bestYears"); if(bestEl) bestEl.innerHTML = "";
    const worstEl = $("worstYears"); if(worstEl) worstEl.innerHTML = "";

    const meta = `대운 시작: ${dt.daeunStart.age}세 전후(${dt.daeunStart.dateApprox}~) / 방향: ${dt.direction}` + (dt.daeunStart.isApprox ? " · 절기근사" : "");
    safeSetText("daeunMeta", meta);

    const root = $("daeunTimeline");
    if(root) renderDaeunDecadeList(root, dt.decades, seedBase);

    // 배지(인생요약)는 간단히: LifeScore만 기준
    const vol = { label:"Normal" };
    const badges = buildGlobalBadges(lifeScore, vol);
    const lifeBadges = $("lifeBadges");
    if(lifeBadges) renderBadges(lifeBadges, badges);
...
function bindUI(){
  ...
  const btn = $("btnRun");
  if(btn) btn.addEventListener("click", (e)=>{ e.preventDefault(); runAll(); });

  // 예시 입력: 빠른 데모용
  const btnExample = $("btnExample");
  if(btnExample) btnExample.addEventListener("click", (e)=>{
    e.preventDefault();
    const name = $("name"); if(name && !name.value) name.value = "홍길동";
    const bd = $("birthDate"); if(bd) bd.value = "1992-01-23";
    const bt = $("birthTime"); if(bt) bt.value = "09:10";
    const g = $("gender"); if(g) g.value = "F";
    runAll();
  });

  // 테스트 실행: 콘솔 스모크 테스트(문제 생기면 alert로 알려줌)
  const btnTests = $("btnTests");
  if(btnTests) btnTests.addEventListener("click", (e)=>{
    e.preventDefault();
    if(typeof window.runTests === "function"){
      const ok = window.runTests();
      setAlert(ok ? "테스트 완료: OK" : "테스트 완료: 일부 실패(콘솔 확인)");
    }else{
      setAlert("테스트 함수(runTests)가 없습니다.");
    }
  });
}
...
// expose for safety
try{ window.renderDaeunDecadeList = renderDaeunDecadeList; }catch(e){}
