...
function getShishen(dayStem, targetStem) {
  ...
  const targetElement = WUXING_STEM[targetStem];
  ...
  if (WUXING_GENERATES[dayElement] === targetElement) {
    return samePolarity ? "食神" : "傷官";
  }
  
  if (WUXING_CONTROLS[dayElement] === targetElement) {
  ...
  if (WUXING_GENERATES[targetElement] === dayElement) {
...
function determineGeok(pillars, vectors) {
  ...
  const hiddenStems = HIDDEN_STEMS_RATIO[monthBranch];
  ...
  for (const clashPair of EARTHLY_CLASHES) {
...
function classifyYongHeeGiHan(state) {
  ...
  const geokPref = GEOK_PREFERENCE[geok.main] || GEOK_PREFERENCE["혼합격"];
  ...
  if (strength.score <= 35) {
    ...
    const inseongElement = Object.keys(WUXING_GENERATES).find(
      e => WUXING_GENERATES[e] === dayElement
    );
    if (inseongElement) {
      Object.keys(WUXING_STEM).forEach(stem => {
        if (WUXING_STEM[stem] === inseongElement) {
          const ss = getShishen(dayStem, stem);
...
function scoreProfile(vectors, interactions, profileName) {
  const profile = PROFILES[profileName] || PROFILES.overall;
  let score = 0;
