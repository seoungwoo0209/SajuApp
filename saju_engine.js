...
function computeTotalScore(state, profileName = "overall") {
  const balanceScore = scoreBalance(state.vectors) * 2; // 0~40
  const strengthScore = scoreStrength(state.strength) * 1.5; // 0~15
  const geokScore = scoreGeokIntegrity(state.geok, state.vectors) * 1.5; // 0~18
  const yhghScore = scoreYongHeeGiHan(state.gods, state.vectors); // 0~18
  const interactionScore = scoreInteractions(state.interactions, state.gods); // -15~+15
  const profileScore = scoreProfile(state.vectors, state.interactions, profileName); // 0~20
  
  const total = balanceScore + strengthScore + geokScore + yhghScore + interactionScore + profileScore;
