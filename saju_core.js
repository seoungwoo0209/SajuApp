...
function monthGanji(utcDate){
  ...
  const branchStr = JIEQI_BRANCH[matchIdx];
  const yg = yearGanji(utcDate);
  const yearStemIdx = STEMS.indexOf(yg.stem);
  
  const branchIdx = BRANCHES.indexOf(branchStr);
  const monthStemIdx = (yearStemIdx*2 + 2 + branchIdx) % 10;
...
function getWuxingCounts(fourPillars, includeHidden){
  ...
  const withHidden = {...counts};
  for(const pillar of [fourPillars.year, fourPillars.month, fourPillars.day, fourPillars.hour]){
    const hiddenStems = HIDDEN_STEMS_BRANCH[pillar.branch];
    if(hiddenStems){
      for(const hs of hiddenStems){
        const wx = WUXING_STEM[hs];
        if(wx) withHidden[wx]++;
      }
    }
  }
  
  return { surface: counts, withHidden };
}
