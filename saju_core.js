...
function runAll(){
  ...
  try{
    ...
    // saju_fortune.js의 총운 분석 함수 호출 (DOM + 스크립트 로딩 대기)
    setTimeout(() => {
      if(window.renderFortuneAnalysis){
        console.log("🚀 renderFortuneAnalysis 호출");
        window.renderFortuneAnalysis();
      } else {
        console.error("❌ renderFortuneAnalysis 함수가 로드되지 않았습니다. saju_fortune.js를 확인하세요.");
        // 재시도
        setTimeout(() => {
          if(window.renderFortuneAnalysis){
            console.log("🚀 [재시도] renderFortuneAnalysis 호출");
            window.renderFortuneAnalysis();
          }
        }, 200);
      }
