...
const { TEN_GODS_KR } = window.SajuData;
const { buildState, computeTotalScore } = window.SajuEngine;
...
function renderGodsInfo(gods) {
  ...
  const formatTenGods = (list) => {
    return list.map(tg => TEN_GODS_KR[tg] || tg).join(", ");
  };
...
function renderDaeunScores(baseState, profileName = "overall") {
  ...
  decades.forEach((decade, idx) => {
    ...
    // 점수 계산
    const scoreResult = computeTotalScore(daeunState, profileName);
...
function onProfileChange(profileName) {
  ...
  const baseState = buildState(window.SajuResult.fourPillars);
  
  // 원국 점수 재계산
  const baseScore = computeTotalScore(baseState, profileName);
