...
function mergeDaeunState(baseState, decade) {
  ...
  const vectors = {
    elements: { ...baseState.vectors.elements },
    tenGods: { ...baseState.vectors.tenGods }
  };
  ...
  // 신강도 재계산 (대운 영향 20% 반영)
  const baseStrength = baseState.strength.score;
  const daeunStrength = window.SajuEngine.calculateStrength(mergedPillars).score;
  const blendedStrength = baseStrength * 0.8 + daeunStrength * 0.2;
  const strength = {
    score: blendedStrength,
    label: blendedStrength >= 66 ? "신강" : blendedStrength <= 35 ? "신약" : "중화",
    breakdown: baseState.strength.breakdown
  };
